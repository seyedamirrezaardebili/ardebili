...
@section('main')
        <form method="post" action="{{ route('adminpanel.product.input') }}"  enctype="multipart/form-data" class='gap-3'>
            @csrf
            @method('delete')
            <input type="hidden" name="id" id="id"  value="{{old('id',$crop['id_id'])}}">
            <h6>ایا مطمن از این کار هستید</h6>
            <button type="submit" class="btn btn-danger">  حذف بشه  </button>
            <a href="{{ route('adminpanel.product') }}"><button type="button" class="btn btn-primary">  برم یک دوری بزنم می ام   </button></a>
